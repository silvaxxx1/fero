...
import { floatingLoveNotes, cursorHearts, heartSky } from './app/romantic-effects';
...
function setupSurprise() {
    ...
    if (!surpriseBtn || !giftBox || !surpriseMessage)
        return;
...
if (playMusicBtn && pauseMusicBtn) {
    ...
    playMusicBtn.addEventListener('click', () => {
        audio.play();
        createInteractiveHeart(playMusicBtn.offsetLeft + playMusicBtn.offsetWidth / 2, playMusicBtn.offsetTop + playMusicBtn.offsetHeight / 2);
    });
...
function createSpicySection() {
    // Use your actual files from /assets/spicy/
    const romanticMedia = [
        ...
        { src: 'skin_to_skin.mp4', caption: 'Skin to Skin ❤️', type: 'video' } // Video
    ];
...
function generateRomanticGallery(mediaArray) {
    ...
    mediaArray.forEach((media, index) => {
        ...
        romanticItem.addEventListener('click', () => {
            ...
            for (let i = 0; i < 10; i++) {
                setTimeout(() => {
                    createInteractiveHeart(romanticItem.offsetLeft + Math.random() * romanticItem.offsetWidth, romanticItem.offsetTop + Math.random() * romanticItem.offsetHeight);
                }, i * 100);
            }
        });
...
function setupSpicyControls() {
    ...
    if (!consentOverlay || !spicyContent || !consentYes || !consentNo || !closeSpicy)
        return;
...
if (imagesContainer) {
    images.forEach((img, index) => {
        ...
        container.addEventListener('click', () => {
            showEnhancedConfetti();
            createInteractiveHeart(container.offsetLeft + container.offsetWidth / 2, container.offsetTop + container.offsetHeight / 2);
        });
...
if (videosContainer) {
    ...
    `;
        ...
        container.addEventListener('mouseenter', () => {
            ...
            for (let i = 0; i < 5; i++) {
                setTimeout(() => {
                    createInteractiveHeart(container.offsetLeft + Math.random() * container.offsetWidth, container.offsetTop + Math.random() * container.offsetHeight);
                }, i * 100);
            }
        });
